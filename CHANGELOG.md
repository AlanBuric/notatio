...
- `seed` pins the random variation, making a drawing reproducible across reloads and in screenshot tests. It is assigned randomly when omitted and readable from the annotation afterwards, so a variation worth keeping can be captured and pasted into the config.
...
- The retreat on `hide()` completes when its animations do, rather than on a `setTimeout` sized to the configured duration. A paused hide now genuinely stays on the page instead of being torn down by a timer that kept running.
