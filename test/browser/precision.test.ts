...
function getDecimalPlaces(value: string): number {
  return value.replace('px', '').split('.')[1]?.length ?? 0;
}
...
describe('numeric precision', () => {
  ...
  it('rounds stroke-dasharray and stroke-dashoffset to at most three decimal places', () => {
    ...
    const { style } = getPathsFor(element)[0];

    expect(getDecimalPlaces(style.strokeDasharray)).toBeLessThanOrEqual(3);
    expect(getDecimalPlaces(style.strokeDashoffset)).toBeLessThanOrEqual(3);
  });
  ...
  it('rounds animation duration and delay to one decimal place of a millisecond', () => {
    ...
    expect(durationMs(style.animationDuration)).toBeCloseTo(333.5, 1);
    expect(durationMs(style.animationDelay)).toBeCloseTo(12.3, 1);
  });
});
